...
def download_and_clip(url, start, end,
    ...
    with VideoFileClip(full) as v:
        ...
        sc.write_videofile(clip, audio_codec="aac", verbose=False)
    return clip
...
def run_inference(clip_path, queries, model, post):
    ...
    clip_v.write_videofile(out_path, audio=True, fps=meta["video_fps"], verbose=False)
    return out_path

# ────────────────────────────────────────────────────────────────────────────────
# 7) Katna‐based summarisation
# ────────────────────────────────────────────────────────────────────────────────
from Katna.video.pipeline import VideoSummarizationPipeline

def summarise_video(video_path, percent, out_dir):
    """
    Use Katna to summarize the video into approximately `percent`% of its original length.
    """
    pipeline = VideoSummarizationPipeline(
        segmentation_method='py-scene-detect',
        summarization_method='clustering',
        summarization_percentage=percent
    )
    ...
    print(f"Katna summary ({percent}%) saved to {summary_path}")
    return summary_path
...
def main():
    ...
    summarized = summarise_video(
        out,
        args.summary_percent,
        args.summary_dir
    )

    print(f"\nFinal MTTR output: {out}")
    print(f"Final summary clip: {summarized}\n")

if __name__ == "__main__":
    """
    Before running, install dependencies:
        pip install torch torchvision yt-dlp moviepy ruamel.yaml gdown katna tqdm einops
    """
    main()
